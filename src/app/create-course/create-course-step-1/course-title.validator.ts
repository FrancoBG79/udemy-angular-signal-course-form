...
//         (c) => c.description.toLocaleLowerCase() === ctx.value()?.toLocaleLowerCase()
//       );
//       return found ?
//         { kind: 'titleExists', message: 'This title is already being used.' } : null;
//     },
//     onError: () => null,
//   })
// }
export function courseTitleExistsAsync(path: SchemaPath<string>) {
  validateAsync<string, string, CourseListResponse>(path, {
    ...
    onSuccess: (result, ctx) => {
      const found = result?.payload?.find((c) => 
          c.description.toLocaleLowerCase() === ctx.value()?.toLocaleLowerCase()
        );
      return found ? { 
        kind: 'titleExists',
